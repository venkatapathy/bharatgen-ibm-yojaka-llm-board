"""Bulk-import BA English BharatGen PDFs into PDF Context and PYQ modules."""
...
BA_ENGLISH_PREFIX = "BharatGen/BA ENGLISH/"
ASSIGNMENT_PREFIX = "BharatGen/ASSIGNMNET QUESTIONS/BA ENGLISH/"
DEFAULT_ZIP = "BharatGen-20260425T162924Z-3-001.zip"


def _course_label(folder_name: str) -> str:
    """BEGC 101_ Indian Classical Literature -> BEGC 101 - Indian Classical Literature"""
    if "_" in folder_name:
        code, title = folder_name.split("_", 1)
...
class Command(BaseCommand):
    help = "Import BA English egyankosh PDFs (PDF Context) and QP/assignment PDFs (PYQ)."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--org",
            default="EduQGen Demo Lab",
            help="Target organization name",
        )
        parser.add_argument(
            "--user",
            default="admin",
            help="User who owns the imported records",
        )
        parser.add_argument(
            "--skip-pdf",
            action="store_true",
            help="Skip PDF Context import",
        )
        parser.add_argument(
            "--skip-pyq",
            action="store_true",
            help="Skip PYQ import",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
            help="List files only; do not create records",
        )
        parser.add_argument(
            "--sync",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        qp_files = self._discover_qp_files(archive)
        assignment_files = self._discover_assignment_files(archive)

        self.stdout.write(f"Found {len(courses)} BA English courses")
        ...
        if options["dry_run"]:
            for course, pdfs in sorted(courses.items()):
                self.stdout.write(f"  PDF Context: {course} ({len(pdfs)} blocks)")
    ...
    def _discover_courses(self, archive: zipfile.ZipFile) -> dict[str, list[str]]:
        ...
        for name in archive.namelist():
            if not name.startswith(BA_ENGLISH_PREFIX) or "/egyankosh/" not in name:
                continue
            if not name.lower().endswith(".pdf"):
                continue
            rel = name[len(BA_ENGLISH_PREFIX) :]
    ...
    def _discover_qp_files(self, archive: zipfile.ZipFile) -> list[str]:
        return sorted(
            name
            for name in archive.namelist()
            if name.startswith(BA_ENGLISH_PREFIX)
            and "/QP/" in name
        ...
        for course_folder, pdf_paths in sorted(courses.items()):
            ...
            with transaction.atomic():
                ctx = PDFContext(
                    ...
                    created_by=user,
                    name=label,
                    description="BharatGen BA English eGyanKosh course material",
        ...
        for path in paths:
            ...
            with transaction.atomic():
                mod = PYQModule(
                    ...
                    created_by=user,
                    name=name,
                    description="BharatGen BA English question paper",
                    file_size_bytes=len(payload),
